...
function buildExercise(spec) {
  ...
  // the figure's facing direction so a side-profile figure's front is obvious.
  // Feet are drawn only when a spec declares `face` (1 = faces right, -1 = left);
  // front-on, top-down and quadruped views omit it and get no feet. Restrict to
  // specific limbs with `footLimbs` (e.g. only the planted leg in a single-leg move).
  const footLs = [];
  if (spec.face) {
    const footNames = spec.footLimbs || names.filter((nm) => /^(leg|shin)/.test(nm));
    for (const nm of footNames) {
      const footPath = (p) => {
        const v = p.limbs[nm], ank = v[v.length - 1];
        return pathOf([ank, [ank[0] + spec.face * (spec.toe || 15), ank[1]]]);
      };
      const fmoves = poses.some((p) => JSON.stringify(footPath(p)) !== JSON.stringify(footPath(poses[0])));
      let fsh;
      if (fmoves) { const kf = seq.map((p, i) => { const o = { t: times[i], s: [footPath(p)] }; if (i < seq.length - 1) { o.i = e_i; o.o = e_o; } return o; }); fsh = { ty: 'sh', nm: nm + '_foot', ks: { a: 1, k: kf } }; }
      else fsh = { ty: 'sh', nm: nm + '_foot', ks: { a: 0, k: footPath(poses[0]) } };
      const FL = layer(0, nm + '_foot', [fsh, gstroke(W, yT, yB), trg()]); FL.op = F; footLs.push(FL);
    }
  }
  ...
  let matL = null;
  if (spec.mat) {
    const [mx, my, mw, mh, mr] = spec.mat;
    matL = layer(0, 'mat', [{ ty: 'rc', nm: 'mr', p: { a: 0, k: [mx, my] }, s: { a: 0, k: [mw, mh] }, r: { a: 0, k: mr } }, stroke(GROUND, 3), trg()]);
    matL.op = F;
  }
...
A({ id: 'prone_neck_extension', headR: 24, face: 1, ground: [[80, 214], [400, 214]], poses: [
...
/* SHOULDER */
A({ id: 'prone_y_raise', headR: 22, mat: [240, 150, 152, 214, 48], poses: [ // overhead: head top, legs trailing, arms in a Y past the head
  { head: [240, 64], limbs: { torso: [[240, 100], [240, 178]], armL: [[240, 106], [212, 72], [192, 50]], armR: [[240, 106], [268, 72], [288, 50]], legL: [[240, 178], [222, 234]], legR: [[240, 178], [258, 234]] } },
  { head: [240, 64], limbs: { torso: [[240, 100], [240, 178]], armL: [[240, 106], [210, 64], [188, 42]], armR: [[240, 106], [270, 64], [292, 42]], legL: [[240, 178], [222, 234]], legR: [[240, 178], [258, 234]] } },
], arrows: [{ dir: 'up', at: [240, 28], phase: 'first' }] });
A({ id: 'prone_t_raise', headR: 22, mat: [240, 150, 152, 214, 48], poses: [ // overhead: arms straight out to the sides (T)
  { head: [240, 64], limbs: { torso: [[240, 100], [240, 178]], armL: [[240, 112], [184, 112]], armR: [[240, 112], [296, 112]], legL: [[240, 178], [222, 234]], legR: [[240, 178], [258, 234]] } },
  { head: [240, 64], limbs: { torso: [[240, 100], [240, 178]], armL: [[240, 112], [182, 102]], armR: [[240, 112], [298, 102]], legL: [[240, 178], [222, 234]], legR: [[240, 178], [258, 234]] } },
], arrows: [{ dir: 'up', at: [184, 84], phase: 'first' }, { dir: 'up', at: [296, 84], phase: 'first' }] });
A({ id: 'prone_w_raise', headR: 22, mat: [240, 150, 152, 214, 48], poses: [ // overhead: bent elbows form a W at shoulder height
  { head: [240, 64], limbs: { torso: [[240, 100], [240, 178]], armL: [[240, 110], [198, 128], [184, 96]], armR: [[240, 110], [282, 128], [296, 96]], legL: [[240, 178], [222, 234]], legR: [[240, 178], [258, 234]] } },
  { head: [240, 64], limbs: { torso: [[240, 100], [240, 178]], armL: [[240, 110], [196, 120], [182, 86]], armR: [[240, 110], [284, 120], [298, 86]], legL: [[240, 178], [222, 234]], legR: [[240, 178], [258, 234]] } },
], arrows: [{ dir: 'up', at: [184, 70], phase: 'first' }, { dir: 'up', at: [296, 70], phase: 'first' }] });
A({ id: 'prone_i_raise', headR: 22, mat: [240, 150, 152, 214, 48], poses: [ // overhead: arms straight overhead in line with the body (I)
  { head: [240, 64], limbs: { torso: [[240, 100], [240, 178]], armL: [[240, 106], [216, 70], [226, 46]], armR: [[240, 106], [264, 70], [254, 46]], legL: [[240, 178], [222, 234]], legR: [[240, 178], [258, 234]] } },
  { head: [240, 64], limbs: { torso: [[240, 100], [240, 178]], armL: [[240, 106], [214, 64], [227, 40]], armR: [[240, 106], [266, 64], [253, 40]], legL: [[240, 178], [222, 234]], legR: [[240, 178], [258, 234]] } },
], arrows: [{ dir: 'up', at: [240, 26], phase: 'first' }] });
...
A({ id: 'prone_scapular_retraction', headR: 22, mat: [240, 150, 152, 214, 48], poses: [ // overhead: arms at sides, shoulder blades squeeze in
...
], arrows: [{ dir: 'up', at: [286, 94], phase: 'first', fixed: true }, { dir: 'down', at: [286, 94], phase: 'second', fixed: true }] });
...
], arrows: [{ dir: 'up', at: [286, 98], phase: 'first', fixed: true }, { dir: 'down', at: [286, 98], phase: 'second', fixed: true }] });
...
A({ id: 'serratus_punch_supine', headR: 22, face: 1, ground: [[70, 214], [410, 214]], poses: [
...
A({ id: 'push_up_plus', headR: 22, face: 1, ground: [[60, 222], [420, 222]], poses: [
...
A({ id: 'side_lying_external_rotation', headR: 22, face: 1, ground: [[70, 220], [410, 220]], poses: [
...
], arrows: [{ dir: 'left', at: [286, 116], phase: 'first', fixed: true }] });
...
], arrows: [{ dir: 'up', at: [292, 72], phase: 'first', fixed: true }] });
...
A({ id: 'prone_extension_superman', headR: 22, face: 1, ground: [[60, 220], [420, 220]], poses: [
...
A({ id: 'swimmers_prone_extension', headR: 22, face: 1, ground: [[60, 220], [420, 220]], poses: [
...
A({ id: 'prone_cobra', headR: 22, face: 1, ground: [[60, 220], [420, 220]], poses: [
...
A({ id: 'cobra_pose_stretch', headR: 22, face: 1, ground: [[60, 220], [420, 220]], poses: [
...
A({ id: 'quadruped_thoracic_rotation', headR: 22, ground: [[60, 220], [420, 220]], poses: [ // one hand stays planted, the other threads under then reaches to the ceiling
  { head: [168, 168], limbs: {
    spine: [[196, 150], [250, 150], [306, 150]],
    armS: [[196, 150], [202, 214]], armM: [[210, 150], [230, 194], [248, 204]],
    leg: [[306, 150], [312, 214]] } },
  { head: [158, 150], limbs: {
    spine: [[196, 150], [250, 150], [306, 150]],
    armS: [[196, 150], [202, 214]], armM: [[210, 150], [236, 118], [258, 90]],
    leg: [[306, 150], [312, 214]] } },
], arrows: [{ dir: 'up', at: [258, 92], phase: 'first' }] });
...
/* CORE */
A({ id: 'dead_bug', headR: 22, face: 1, footLimbs: ['shinM'], ground: [[70, 220], [410, 220]], poses: [ // both arms up and both knees bent; one arm + the opposite leg lower
  { head: [150, 198], limbs: {
    spine: [[178, 202], [260, 202]],
    armS: [[196, 202], [196, 156]], armM: [[204, 202], [204, 156]],
    thighS: [[260, 202], [284, 172]], shinS: [[284, 172], [282, 140]],
    thighM: [[260, 202], [292, 174]], shinM: [[292, 174], [292, 142]] } },
  { head: [150, 198], limbs: {
    spine: [[178, 202], [260, 202]],
    armS: [[196, 202], [196, 156]], armM: [[204, 202], [180, 168]],
    thighS: [[260, 202], [284, 172]], shinS: [[284, 172], [282, 140]],
    thighM: [[260, 202], [302, 196]], shinM: [[302, 196], [334, 200]] } },
], arrows: [{ dir: 'down', at: [230, 150], phase: 'always' }] });
A({ id: 'supine_marching', headR: 22, face: 1, footLimbs: ['shinM'], ground: [[70, 220], [410, 220]], poses: [ // both knees up; one foot lowers to tap the floor and returns
  { head: [150, 198], limbs: {
    spine: [[178, 202], [260, 202]], arm: [[178, 202], [150, 210]],
    thighS: [[260, 202], [286, 172]], shinS: [[286, 172], [284, 142]],
    thighM: [[260, 202], [294, 174]], shinM: [[294, 174], [294, 144]] } },
  { head: [150, 198], limbs: {
    spine: [[178, 202], [260, 202]], arm: [[178, 202], [150, 210]],
    thighS: [[260, 202], [286, 172]], shinS: [[286, 172], [284, 142]],
    thighM: [[260, 202], [300, 196]], shinM: [[300, 196], [330, 202]] } },
], arrows: [{ dir: 'down', at: [300, 168], phase: 'first' }] });
A({ id: 'bird_dog', headR: 22, ground: [[60, 220], [420, 220]], poses: [ // on all fours, extend opposite arm and leg while the other two stay planted
  { head: [168, 168], limbs: {
    spine: [[196, 152], [252, 150], [306, 152]],
    armS: [[194, 152], [198, 214]], armM: [[202, 152], [210, 214]],
    legS: [[308, 152], [314, 214]], legM: [[302, 152], [308, 214]] } },
  { head: [168, 166], limbs: {
    spine: [[196, 152], [252, 150], [306, 152]],
    armS: [[194, 152], [198, 214]], armM: [[202, 152], [150, 138]],
    legS: [[308, 152], [314, 214]], legM: [[302, 152], [360, 126]] } },
], arrows: [{ dir: 'left', at: [120, 110], phase: 'first' }, { dir: 'right', at: [380, 112], phase: 'first' }] });
A({ id: 'standing_bird_dog', headR: 24, face: -1, footLimbs: ['legS'], poses: [ // hinged, faces forward (left); foot only on the planted stance leg
...
A({ id: 'front_plank', headR: 22, face: 1, ground: [[60, 222], [420, 222]], poses: [
...
A({ id: 'plank_shoulder_taps', headR: 22, face: 1, ground: [[60, 222], [420, 222]], poses: [
...
A({ id: 'posterior_pelvic_tilt', headR: 22, face: 1, ground: [[70, 220], [410, 220]], poses: [
...
A({ id: 'curl_up', headR: 22, face: 1, ground: [[70, 220], [410, 220]], poses: [
...
A({ id: 'breathing_90_90', headR: 22, face: 1, ground: [[70, 220], [410, 220]], poses: [
...
/* HIP */
A({ id: 'single_leg_glute_bridge', headR: 22, face: 1, footLimbs: ['legD'], ground: [[70, 214], [410, 214]], poses: [ // foot only on the planted (bent) leg, not the lifted one
...
A({ id: 'prone_hip_extension', headR: 22, face: 1, footLimbs: ['legS'], ground: [[60, 220], [420, 220]], poses: [ // foot only on the grounded straight leg, not the lifted one
...
A({ id: 'figure_4_stretch', headR: 22, face: 1, footLimbs: ['shin'], ground: [[70, 220], [410, 220]], poses: [ // foot on the planted supporting leg only
...
A({ id: 'hip_flexor_lunge_stretch', headR: 24, face: -1, ground: [[60, 220], [420, 220]], poses: [ // faces forward over the front (left) leg
...
], arrows: [{ dir: 'left', at: [196, 150], phase: 'first' }] });
A({ id: 'standing_hip_flexor_stretch', headR: 24, face: -1, ground: [[60, 220], [420, 220]], poses: [ // faces forward over the front (left) leg
...
], arrows: [{ dir: 'left', at: [188, 150], phase: 'first' }] });
...
A({ id: 'glute_bridge', headR: 25, face: 1, ground: [[70, 214], [320, 214]], poses: [
