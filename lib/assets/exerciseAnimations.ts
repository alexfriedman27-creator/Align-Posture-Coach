// Single source of truth for exercise animations.
//
// LIBRARY: Lottie (lottie-react-native) — one library for all 56 exercises so
// the motion style stays consistent. Theming (dark surface + category accent)
// is enforced by components/shared/ExerciseAnimation.tsx, so any animation
// dropped in here renders inside the same app-matching frame.
//
// Metro needs every require() arg to be a string literal, so each exercise gets
// its own line. TO ADD AN ANIMATION:
//   1. Drop <exercise_id>.json into assets/animations/exercises/
//   2. Delete the leading `//` on that exercise's line below.
// Any exercise without an entry falls back to the themed placeholder (never
// crashes). Custom exercises always fall back.
//
// All 56 built-in exercise ids are pre-listed below, grouped by slot.
const EXERCISE_ANIMATION_MAP: Record<string, ReturnType<typeof require>> = {
  ...
  curl_up:               require('../../assets/animations/exercises/curl_up.json'),
  breathing_90_90:       require('../../assets/animations/exercises/breathing_90_90.json'),
  ...
  prone_hip_extension:         require('../../assets/animations/exercises/prone_hip_extension.json'),
  figure_4_stretch:            require('../../assets/animations/exercises/figure_4_stretch.json'),
  ...
  brugger_relief:             require('../../assets/animations/exercises/brugger_relief.json'),
};
